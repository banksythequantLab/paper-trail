...
run_checks() is importable: ingest/emit_assertions.py reuses it to publish the
same results as native DataHub assertions on the evidence datasets.
...
def run_checks(warehouse=None, golden=None):
    """Re-derive every golden value from the warehouse and return a list of
    (name, ok, detail) tuples. Pure: read-only connection, no process exit -- so
    both the CI gate (main) and the assertion publisher can consume it."""
    con = duckdb.connect(warehouse or WAREHOUSE, read_only=True)
    q = lambda s: con.execute(s).fetchall()
    g = yaml.safe_load(open(golden or GOLDEN, encoding="utf-8"))
    checks = []
